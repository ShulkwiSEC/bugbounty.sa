...
**Why Markdown is converted:** bugbounty.sa edits reports in a Quill rich-text editor and stores HTML, so a raw Markdown body renders as literal `**asterisks**` on the platform. Quill is a WYSIWYG editor with no Markdown parser: formatting comes from its toolbar, shortcuts, or pasted rendered rich text. bbsa converts to the tag set that toolbar produces — `h3`/`h4` (every Markdown heading level folds into those two), `strong`, `s`, fenced code blocks, blockquotes, ordered and bullet lists, and links — so a pushed report stays editable in the web UI. `*italic*` and inline `` `code` `` become `em`/`code`, which render but have no toolbar button. Nested lists flatten to one level and horizontal rules are dropped. The 5000-character cap counts stored HTML, so escaped `&lt;` in code-heavy sections adds up fast; `push --dry-run` shows the exact HTML. Add the PoC and evidence with repeatable `--attach PATH` flags when drafting; files upload only during the operator-gated push.
